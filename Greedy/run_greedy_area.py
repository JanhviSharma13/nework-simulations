#!/usr/bin/env python3
"""
Greedy Algorithm Runner for SUMO Networks
...
from build_greedy_routes import generate_routes
import matplotlib.pyplot as plt
import numpy as np

def process_area(area_name, net_file, trips_file, output_dir):
    """Process a single area with Greedy algorithm"""
    print(f"\n{'='*60}")
    ...
    # File paths
    routes_file = os.path.join(output_dir, f"{area_name.lower()}_greedy_routes.rou.xml")
    
    # Step 1: Build Greedy routes
    print(f"Step 1: Building Greedy routes for {area_name}...")
    try:
        ...
        import glob
        metrics_files = glob.glob(f"greedy_metrics_{area_name.lower()}_*.csv")
        if metrics_files:
            latest_metrics = max(metrics_files, key=os.path.getctime)
            new_metrics_path = os.path.join(output_dir, f"{area_name.lower()}_greedy_metrics.csv")
...
def generate_plots(metrics, area_name, output_dir):
    ...
    plt.scatter(df['total_distance'], df['total_time'], color='green', alpha=0.6)
    ...
    plt.title(f"Trip Time vs Distance - {area_name} (Greedy)")
    ...
    # Plot 2: Greedy Performance Metrics
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 8))
    
    # Nodes expanded vs route length
    ax1.scatter(df['route_length'], df['nodes_expanded'], alpha=0.6, color='green')
    ...
    ax2.scatter(df['search_depth'], df['nodes_expanded'], alpha=0.6, color='green')
    ...
    ax3.hist(df['route_length'], bins=20, alpha=0.7, color='green')
    ...
    ax4.hist(df['total_distance'], bins=20, alpha=0.7, color='green')
...
def main():
    ...
    print(f"\n{'='*60}")
    print("GREEDY ALGORITHM RUNNER COMPLETE")
    print(f"{'='*60}")
