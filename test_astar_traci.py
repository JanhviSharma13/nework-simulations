#!/usr/bin/env python3
"""
Test script to verify the fixed A* TraCI simulation
"""

import sys
import os
sys.path.append('A-Star')

from astar_agent import build_graph, a_star_path, run_simulation
import xml.etree.ElementTree as ET

def test_astar_traci():
    """Test the fixed A* TraCI simulation with a single trip"""
    ...
    print(f"Testing A* trip: {from_edge} -> {to_edge}")
    
    # Run A* algorithm
    edges, path_metrics = a_star_path(G, net, from_edge, to_edge)
    
    if edges and path_metrics['success']:
        print(f"✅ A* path found with {len(edges)} edges")
        ...
        if sim_metrics['success']:
            ...
            print(f"❌ A* TraCI simulation failed: {sim_metrics.get('error', 'Unknown error')}")
        
        # Clean up
        if os.path.exists(temp_route_file):
            os.remove(temp_route_file)
    else:
        print(f"❌ No A* path found: {path_metrics.get('error', 'Unknown error')}")
